...
export function buildApp(options: AppOptions): FastifyInstance {
  ...
  app.get('/api/places/candidates', async (req, reply) => {
    ...
    const [lonMin, latMin, lonMax, latMax] = bbox.split(',').map(Number);
    if (bbox.split(',').length !== 4 || [lonMin, latMin, lonMax, latMax].some((n) => !Number.isFinite(n)) || !(lonMin! < lonMax!) || !(latMin! < latMax!)) {
      return reply.code(400).send({ error: 'invalid bbox' });
    }
    const res = await options.pool.query(
      `select id, name, category, source,
              ...
              public_profile, verification_status, corroboration
       from places
  ...
  app.get('/api/places/:id', async (req, reply) => {
    ...
    const res = await options.pool.query(
      `select
         ...
         p.public_phone, p.public_website, p.public_socials, p.public_address, p.public_source, p.public_subcategory, p.public_profile, p.contact_confirmed_at,
